...
import { ChevronDown, ChevronUp, Key, Utensils, Thermometer, Trash2 } from 'lucide-react';
import { MANUAL_SECTIONS } from '../constants';

const IconMap: Record<string, any> = {
  Key, Utensils, Thermometer, Trash2
};
    ...
    <div className="space-y-4 pb-24 animate-in slide-in-from-right duration-500">
      ...
      {MANUAL_SECTIONS.map((section) => {
        ...
        return (
          <div 
            key={section.id} 
            className={`glass rounded-2xl overflow-hidden transition-all duration-300 shadow-sm ${
              isExpanded ? 'ring-1 ring-earth/40' : ''
            }`}
          >
            <button
            ...
            >
              <div className="flex items-center gap-4">
                <div className={`p-2 rounded-xl transition-colors duration-300 ${
                  isExpanded ? 'bg-earth text-white' : 'bg-earth/10 text-earth'
                }`}>
                  <Icon size={20} />
                </div>
